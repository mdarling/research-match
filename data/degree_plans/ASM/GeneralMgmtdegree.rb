ugp = UndergradProgram.where(:name=>'General Management',:degree=>'BBA')[0]
...
#Course:No Concentration Elective Term:7
DegreeRequirement.create(:academic_program_id => ugp.id,
:name => 'No Concentration Elective',
:credits =>6,
:min_grade => 'C-',
:crucial =>false)
...
# linking Courses
# Id for General Management
# Id of General Management Curriculum for academic year 2013-2014
...
c22 = Course.where(:number => 'MGMT 326', :dept_id => Dept.where(:acronym => 'ASM')[0].id)[0]
c22.degree_requirements << DegreeRequirement.where(:name => 'MGMT 326: Financial Management', :academic_program_id => ugp.id)

c23 = Course.where(:number => 'MGMT 450', :dept_id => Dept.where(:acronym => 'ASM')[0].id)[0]
c23.degree_requirements << DegreeRequirement.where(:name => 'MGMT 450: Computer-Based Information Systems', :academic_program_id => ugp.id)
...
c27 = Course.where(:number => 'MGMT 498', :dept_id => Dept.where(:acronym => 'ASM')[0].id)[0]
c27.degree_requirements << DegreeRequirement.where(:name => 'MGMT 498: Strategic Management ', :academic_program_id => ugp.id)
...
t7.degree_requirements << DegreeRequirement.where(:name => 'No Concentration Elective', :academic_program_id => ugp.id,:credits=>6)
...
t8.degree_requirements << DegreeRequirement.where(:name => 'No Concentration Elective', :academic_program_id => ugp.id,:credits=>6)
